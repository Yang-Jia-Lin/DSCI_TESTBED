...
from Src.Shared.Config.paths import RESULT_ABLATION_PATH
...
def plot_bubble_chart(data: pd.DataFrame, save_dir=Path(RESULT_ABLATION_PATH)):
    ...
    SEMANTIC_COLORS = {
        ...
        "Ours": COLORS["red"],
        "Single Device / no EE": COLORS["grey"],
        "Single Device / with EE": COLORS["blue"],
        "Single Edge / no EE": COLORS["brown"],
        "Single Edge / with EE": COLORS["blue"],
        "Single Cloud / no EE": COLORS["green"],
        "Single Cloud / with EE": COLORS["blue"],
        "Two-tier Device-Edge / no EE": COLORS["purple"],
        "Two-tier Device-Edge / with EE": COLORS["blue"],
        "Two-tier Edge-Cloud / no EE": COLORS["purple"],
        "Two-tier Edge-Cloud / with EE": COLORS["blue"],
        "Three-tier Device-Edge-Cloud / no EE": COLORS["red"],
        "Three-tier Device-Edge-Cloud / with EE": COLORS["blue"],
        "Ours / optimized split + EE": COLORS["red"],
    }
    name_map = {
        "仅在终端": "Device",
        "仅在边端": "Edge",
...
def plot_utility_bar(data: pd.DataFrame, save_dir=Path(RESULT_ABLATION_PATH)):
    ...
    colors = [SEMANTIC_COLORS.get(name, "#888888") for name in data["display_name"]]
    bars = ax2.bar(

...
from Scripts.Exp5_System_Overhead.plot_convergency import (
    plot_convergence,
    plot_entropy,
    plot_lan_and_acc,
)
...
def run_ppo_vs_baselines(
    *,
    ...
    outputs["optimizer_comparison_fig"] = plot_optimizer_comparison(old_style, target_dir)
    return outputs
...
def run_observed_device_summary(
    *,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    grouped = (
        frame.groupby(["round_id", "exit_location"], dropna=False)
        .agg(
...
def write_exp3_summary(*, output_dir: str | Path, outputs: dict[str, Path]) -> Path:
    ...
    lines.append(f"- Component shares sum to {_fmt(latency['share_of_total'].sum(), 6)}.")
    ...
    if observed.empty:
        ...
        locations = ", ".join(sorted(str(value) for value in observed["exit_location"].dropna().unique()))
        lines.append(

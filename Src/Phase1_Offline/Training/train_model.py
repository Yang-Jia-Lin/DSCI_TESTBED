...
from Src.Phase1_Offline.Training.runtime import atomic_save, checkpoint_payload, cosine_warmup, restore, run_epoch, seed_all, write_environment, copy_split_metadata, cpu_state
...
def main(argv=None):
 ...
 start=0; best=-1.; bad=0; best_model=cpu_state(model); log_path=paths.analysis_root/"train_model_log.csv"; logs=pd.read_csv(log_path).to_dict("records") if a.resume and log_path.is_file() else []
 ...
 while epoch<a.epochs and bad<a.patience:
  ...
  scheduler.step(); logs.append(dict(stage="final",epoch=epoch+1,train_loss=train_loss,train_acc=train_acc,val_loss=val_loss,val_acc=val_acc,best_val_acc=best,batch_size=batch,accumulation=accumulation))
 ...
 model.load_state_dict(best_model); atomic_save(model.state_dict(),paths.weight_path); pd.DataFrame(logs).to_csv(log_path,index=False)

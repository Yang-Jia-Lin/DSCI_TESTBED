...
from Src.Shared.Utils.training_log import save_model_weights, save_train_log
...
from Src.Shared.Models.ModelNet.Resnet50 import Bottleneck, MultiEEResNet50, freeze_layers
from Src.Shared.Config.paths import DATA_ROOT, RESULT_EE_MODEL_PATH, WEIGHTS_DIR
...
def run_stage(
    ...
    for ep in range(1, num_epochs + 1):
        tr_loss, tr_acc = train_one_epoch(
            model, train_loader, optimizer, stage_tag, device, criterion
        )
        vl_loss, vl_acc = validate_one_epoch(
            model, valid_loader, stage_tag, device, criterion
        )
...
if __name__ == "__main__":
    ...
    run_stage(
        ...
        stage_name="Stage‑2",
        freeze_kwargs=dict(
            freeze_backbone=True, freeze_x3_fc=True
        ),  # backbone & x3_fc 锁
        stage_tag="x2_fc",
        num_epochs=epochs_stage,
    )
    ...
    run_stage(
        ...
        stage_name="Stage‑3",
        freeze_kwargs=dict(
            freeze_backbone=True, freeze_x2_fc=True
        ),  # backbone & x2_fc 锁
        stage_tag="x3_fc",
        num_epochs=epochs_stage,
    )

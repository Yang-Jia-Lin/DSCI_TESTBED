...
def build_cifar10_test_transform():
    ...
    try:
        ...
        # Some Pillow versions expose resampling filters under Image (older)
        # while newer versions provide the Resampling enum. Use a safe
        # getattr fallback to avoid attribute errors.
        resample = getattr(Image, "BILINEAR", Image.Resampling.NEAREST)
...
def iter_cifar10_test_samples(
    data_root: str | Path = MODEL_PATHS.data_dir / MODEL_CFG.dataset_name / "cifar-10-batches-py",
    difficulty_table_path: str | Path | None = None,
    difficulty: str | Iterable[str] | None = None,
    include_difficulty_metadata: bool = False,
    include_image_id: bool = False,

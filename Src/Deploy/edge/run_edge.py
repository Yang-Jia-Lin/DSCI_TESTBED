...
def status():
    return jsonify({"f_e_max": get_max_cpu(), "BW_e2c": measure_bandwidth_iperf("127.0.0.1", 5002)})


def run_feature_server():
    me = torch.load("Models/Weights/me.pth",
...
if __name__ == "__main__":
    threading.Thread(target=lambda: status_app.run(
        host="0.0.0.0", port=9002, debug=False, use_reloader=False), daemon=True).start()
    run_feature_server()

...
def split_and_save(state_dict_path, save_dir: str | Path = WEIGHTS_DIR):
    ...
    # 边缘段：layer3
    me = nn.Sequential(OrderedDict([("layer3", model.layer3)]))
    ...
    torch.save(
        model.fc2.state_dict(), os.path.join(save_dir, "exit1_fc.pth")
    )  # 设备端早退
    torch.save(
        model.fc3.state_dict(), os.path.join(save_dir, "exit2_fc.pth")
    )  # 边缘端早退
...
if __name__ == "__main__":
    # 替换为你的其中一个 .pth 文件路径
    split_and_save(WEIGHTS_DIR / "ResNet50_multi_EE_model.pth")

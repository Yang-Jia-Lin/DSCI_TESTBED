...
def flatten_model(model):
    ...
    def _add_module(module, prefix=""):
        ...
        if isinstance(module, (nn.Conv2d, nn.BatchNorm2d, nn.ReLU, nn.MaxPool2d,
                               nn.AdaptiveAvgPool2d, nn.Linear, nn.Flatten)):
            # 为层分配全局索引
            ...
            for child_name, child in module.named_children():
                _add_module(
                    child, f"{prefix}.{child_name}" if prefix else child_name)
...
def main():
    ...
    # 你的权重文件路径 (选一个)
    weight_path = "C:\\Users\\t1960\\Desktop\\Code\\DSCI_TESTBED-main\\Src\\models\\ResNet50_multi_EE_model.pth"   # 请替换为实际路径
    ...
    torch.save(flat_layers, os.path.join(save_dir, "all_layers.pth"))

    # 4. 保存早退分类器权重 (fc2 和 fc3)
    # 注意：fc2 在 layer2 之后，fc3 在 layer3 之后，它们的权重直接提取
    torch.save(model.fc2.state_dict(), os.path.join(save_dir, "exit1_fc.pth"))
    torch.save(model.fc3.state_dict(), os.path.join(save_dir, "exit2_fc.pth"))

    # 5. 打印层索引与名称对照表（可写入文件）
    with open("Models/Weights/layer_mapping.txt", "w") as f:
